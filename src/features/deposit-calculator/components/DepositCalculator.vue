...
<template>
  <AppCard class="grid gap-0.5 my-4">
    ...
    <AppInput
      label="Data założenia lokaty"
      id="start-date"
      type="date"
      v-model="data.startDate"
      :disabled="disableInputs"
      :error="formValidation.fields.startDate"
    />

    <AppInput
      label="Oprocentowanie w skali roku (w procentach)"
      id="annual-interest"
      type="number"
      v-model.number="data.interest"
      step="0.5"
      :disabled="disableInputs"
      :error="formValidation.fields.interest"
      min="0"
    />

    <AppInput
      label="Okres czasu (w miesiącach)"
      id="period-months"
      type="number"
      v-model.number="data.periodMonths"
      :disabled="disableInputs"
      :error="formValidation.fields.periodMonths"
      min="1"
    />
  ...
  <AppCard>
    ...
    <div v-else>W formularzu są błędy.</div>
  </AppCard>
</template>
