...
const { depositFullInfo: d } = defineProps<{ depositFullInfo: DepositFullInfo }>()

const title =
    `Zyski: brutto - ${formatCurrency(d.results.profitsProjected.gross)}, netto - ${formatCurrency(d.results.profitsProjected.net)}`

</script>
<template>
    <AppLink :to="`/lokaty/${d.info.id}`" :title="title">
